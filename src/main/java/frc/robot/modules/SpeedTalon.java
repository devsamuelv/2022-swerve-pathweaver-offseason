...
public class SpeedTalon extends TalonFX {

  public SpeedTalon(int deviceNumber) {
    super(deviceNumber);

    /* Config the peak and nominal outputs, 12V means full */
    this.configNominalOutputForward(0, SpeedConstants.kTimeoutMs);
    this.configNominalOutputReverse(0, SpeedConstants.kTimeoutMs);
    this.configPeakOutputForward(1, SpeedConstants.kTimeoutMs);
    this.configPeakOutputReverse(-1, SpeedConstants.kTimeoutMs);

    /**
     * Config the allowable closed-loop error, Closed-Loop output will be neutral
     * within this range. See Table in Section 17.2.1 for native units per rotation.
     */
    this.configAllowableClosedloopError(0, SpeedConstants.kPIDLoopIdx, SpeedConstants.kTimeoutMs);

    /* Config Position Closed Loop gains in slot0, tsypically kF stays zero. */
    this.config_kF(SpeedConstants.kPIDLoopIdx, SpeedConstants.kGains.kF, SpeedConstants.kTimeoutMs);
    this.config_kP(SpeedConstants.kPIDLoopIdx, SpeedConstants.kGains.kP, SpeedConstants.kTimeoutMs);
    this.config_kI(SpeedConstants.kPIDLoopIdx, SpeedConstants.kGains.kI, SpeedConstants.kTimeoutMs);
    this.config_kD(SpeedConstants.kPIDLoopIdx, SpeedConstants.kGains.kD, SpeedConstants.kTimeoutMs);
  }
